...
const Home: React.FC = () => {
    ...
    if (status === "idle" || records.length === 0) {
      ...
      const fetchData = async () => {
        try {
          ...
          //localStorage.setItem("analytics", JSON.stringify(initialCardAnalytics));
      ...
      //   const storedCardAnalytics = localStorage.getItem("analytics");
      //   if (storedCardAnalytics) {
      // 	const parsedAnalytics = JSON.parse(storedCardAnalytics);
      // 	dispatch(fetchCardAnalyticsSuccess(parsedAnalytics));
      //} else {
      fetchData();
      //}
    } else {
  ...
  const defineCardanalytics = (data: any) => {
	const initialCardAnalytics: CardAnalytics[] = data.map(
		(post: Post) => ({
		  cardId: post.id,
